  ...
  return (
    <DropdownMenu open={open} onOpenChange={setOpen}>
      {isMobile ? (
        <TooltipProvider>
          <Tooltip>
            <DropdownMenuTrigger render={<TooltipTrigger render={<Button variant="outline" size="icon" aria-label={buttonLabel} {...rest} />} />}>{icon}</DropdownMenuTrigger>
            <TooltipContent>
              <p>{buttonLabel}</p>
            </TooltipContent>
          </Tooltip>
        ...
        <DropdownMenuTrigger render={<Button variant="outline" size="sm" className="h-9" {...rest} />}>{icon}<span className="ml-2">{buttonLabel}</span><ChevronDown className="ml-2 h-4 w-4" /></DropdownMenuTrigger>
      )}
