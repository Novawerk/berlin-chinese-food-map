      ...
      <FormField className={props.className} id={id} name={field.name}>
        ...
        <FormControl>
          <Popover open={open} onOpenChange={handleOpenChange} modal={modal}>
            <PopoverTrigger render={<Button variant="outline" role="combobox" aria-expanded={open} className="w-full justify-between h-auto py-1.75 font-normal" />}>{selectedChoice ? (
                                        getInputText(selectedChoice)
                                      ) : (
                                        <span className="text-muted-foreground">{placeholder}</span>
                                      )}<ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" /></PopoverTrigger>
